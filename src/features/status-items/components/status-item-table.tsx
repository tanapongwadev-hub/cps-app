...
export function StatusItemTable({ items, isLoading, page, pageSize, totalItems, totalPages, onPageChange, onPageSizeChange, onEdit, onStatusChange }: {
  items: StatusItem[]; isLoading: boolean;
  page: number; pageSize: number; totalItems: number; totalPages: number;
  onPageChange: (p: number) => void; onPageSizeChange: (s: number) => void;
  onEdit: (s: StatusItem) => void; onStatusChange: (s: StatusItem) => void;
}) {
  return (
    <Card className="p-4">
      <DataTable
        data={items} isLoading={isLoading} globalSearch={false}
        ...
        columns={[
          ...
          { id: "module", header: "โมดูล", cell: ({ row }: any) => <Badge variant="outline">{row.original.module}</Badge> },
          { id: "color", header: "สี", cell: ({ row }: any) => <Badge variant={row.original.color as any}>{row.original.color}</Badge> },
